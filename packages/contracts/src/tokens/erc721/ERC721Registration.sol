...
contract ERC721Registration {
  function install(IBaseWorld world, bytes16 namespace, string memory _name, string memory _symbol, ERC721System token) public {
  ...
  function registerTables(IBaseWorld world, bytes16 namespace) private returns (bytes32 metadataTableId){
    ...
    metadataTableId = world.registerTable(namespace, METADATA_T, MetadataTable.getSchema(), MetadataTable.getKeySchema());
  }
}
